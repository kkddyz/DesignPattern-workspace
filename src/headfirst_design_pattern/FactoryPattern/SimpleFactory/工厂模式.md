...
>变化又出现了，这一次是"风味";
>
>不同的PizzaStore，需要的pizza的风味不同,风味指什么？？？
>
>其实就是对象的成员变量，不同风味意味着：  酱料，芝士，博饼，变量的值不同
...
**框架的目的是让披萨制作活动的范围局限在PizzaStore类，保证制作pizza使用同一套框架**

**而同时又能让这些加盟店自由的制作该区域的风味(决定制作的pizza的实现类)**
...
工厂方法处理对象创建，并将这样的行为封装在子类中。这样客户程序中关于超类的代码(orderPizza())就和子类对象的创建代码解耦了。
